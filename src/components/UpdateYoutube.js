...
class UpdateYoutube extends Component {
    ...
    updateYouTube = () => {
        ...
        axios.get('https://cors-anywhere.herokuapp.com/https://ebenezer-final-server.now.sh/live')
            ...
            <Router>
                <Switch>
                    <div className='update-youtube container'>
                        ...
                        {this.state.submitted ? null : <button onClick={this.updateYouTube} className='btn btn-outline-secondary'><img alt='youtube-img' src={youtubeIMG}></img>Actualizar Video</button> }
                        { this.state.load ? <Loading/> : null }
                        {this.state.upToDate ? <div><h4>El video ya está actualizado</h4> <button onClick={this.redirectToSermons} className='btn btn-outline-secondary'><img alt='sermonsIcon' src={sermonsIcon}></img>Ir a Sermones</button></div> : null }
                    </div>
                </Switch>
            </Router>
        );
