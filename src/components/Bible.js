...
class Bible extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevState.currentPage !== this.state.currentPage) {
      ...
      axios
        .get(
          `https://api.scripture.api.bible/v1/bibles/592420522e16049f-01/books/${this.state.currentPage}?include-chapters=true`,
          { headers: { "api-key": "7dbe0c4776c211bd9f6bdfe5b3f6694a" } }
        )
        .then((response) => {
    ...
    if (prevState.currentChapter !== this.state.currentChapter) {
      ...
      axios
        .get(
          `https://api.scripture.api.bible/v1/bibles/592420522e16049f-01/chapters/${this.state.currentChapter}/verses`,
          { headers: { "api-key": "7dbe0c4776c211bd9f6bdfe5b3f6694a" } }
        )
        .then((response) => {
          // Getting the verses
    ...
    if (prevState.currentVerse !== this.state.currentVerse) {
      ...
      axios
        .get(
          `https://api.scripture.api.bible/v1/bibles/592420522e16049f-01/verses/${pickedVerse}?content-type=text&include-notes=false&include-titles=true&include-chapter-numbers=false&include-verse-numbers=true&include-verse-spans=false&use-org-id=false`,
          { headers: { "api-key": "7dbe0c4776c211bd9f6bdfe5b3f6694a" } }
        )
        .then((response) => {
          // getting the content
          ...
          <div className="bible-nav">
            ...
            <div className="search-container" id="search-container">
              <div className="search-bar">
                <input
                  onBlur={this.NoSearch}
                  type="text"
                  id="search-input"
                ></input>
                <button onClick={this.Search}>
                  <img alt="search-img" src={searchImg}></img>
                </button>
              </div>
            </div>
          </div>
            ...
            <Switch>
              ...
              <Route exact path="/bible/chapter">
                <ChapterPicker
                  ...
                  onChange={this.chapterEventHandler}
                />
              </Route>

              <Route exact path="/bible/chapter/verse">
                <VerseViewer
                  verseData={this.state.verseData}
                  verseCount={this.state.bookData}
                  onChange={this.verseEventHandler}
                />
              </Route>
            </Switch>
          </div>
